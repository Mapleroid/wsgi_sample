...
class Router(object):
    ...
    def _dispatch(req):
        ...
        if not match:
            implemented_http_methods = ['GET', 'POST', 'DELETE']
...
class Resource(object):
    ...
    def __call__(self, request):
        ...
        #body_expect = strutils.bool_from_string(
        #    action_args.pop('body_expect', None))
        action_args.pop('body_reject', None)
        action_args.pop('body_expect', None)
        body_reject = False
        body_expect = False
        ...
        try:
            if body_reject and self.deserializer.has_body(request):
                msg = 'A body is not expected with this request.'
                raise webob.exc.HTTPBadRequest(explanation=msg)
			
            if body_expect and not self.deserializer.has_body(request):
                msg = 'A body is expected with this request.'
                raise webob.exc.HTTPBadRequest(explanation=msg)
